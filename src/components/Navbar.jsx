  ...
  return (
    <header style={{ background: "var(--green)", boxShadow: "0 2px 0 var(--green-deep)", position: "sticky", top: 0, zIndex: 30 }}>
      <div style={{ maxWidth: 1120, margin: "0 auto", padding: "12px clamp(16px, 4vw, 36px)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16 }}>
        <div className="reci-header-left" style={{ display: "flex", alignItems: "center", gap: 22 }}>
          <button type="button" onClick={() => navigate(links[0].path)} style={{ display: "flex", alignItems: "center", gap: 11, background: "none", border: "none", cursor: "pointer", padding: 0, flexShrink: 0 }}>
          ...
          <nav className="reci-nav no-bar" style={{ display: "flex", gap: 4 }}>
            {links.map((l) => {
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
          <button type="button" onClick={() => navigate("/perfil")} style={{ display: "flex", alignItems: "center", gap: 9, background: "none", border: "none", cursor: "pointer", padding: 0 }}>
            <span style={{ width: 36, height: 36, borderRadius: "50%", background: "var(--cream)", color: "var(--green-deep)", display: "grid", placeItems: "center", fontFamily: "var(--serif)", fontSize: 17, flexShrink: 0 }}>{initial}</span>
            <span className="reci-user-text" style={{ textAlign: "left", lineHeight: 1.1 }}>
          ...
          }}><Icon name="logout" size={16} stroke="var(--green-deep)" /><span className="reci-logout-text">Salir</span></button>
        </div>
      </div>
    </header>
  );
